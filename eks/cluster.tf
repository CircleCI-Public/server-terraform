...
resource "aws_iam_policy" "aws-lb-controller" {
  // Policy definition from AWS: https://docs.aws.amazon.com/eks/latest/userguide/aws-load-balancer-controller.html
  name        = "${var.basename}-aws-lb-controller"
  description = "Used by the AWS Load Balancer Controller"
  policy      = <<-EOF
    {
        "Version": "2012-10-17",
        "Statement": [
